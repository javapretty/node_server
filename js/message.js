
if (typeof Msg == "undefined") {
	var Msg = {};
	Msg.REQ_HEARTBEAT = 0; //发送心跳到gate_server
	Msg.REQ_SELECT_GATE = 1; //选择gate_server
	Msg.REQ_CONNECT_GATE = 2; //登录gate_server
	Msg.REQ_FETCH_ROLE = 3; //获取角色
	Msg.REQ_CREATE_ROLE = 4; //创建角色
	Msg.REQ_FETCH_MAIL = 5; //获取邮件
	Msg.REQ_PICKUP_MAIL = 6; //收取附件
	Msg.REQ_DEL_MAIL = 7; //删除邮件
	Msg.REQ_FETCH_BAG = 8; //获取背包
	Msg.REQ_CREATE_GUILD = 201; //创建公会
	Msg.REQ_DISSOVE_GUILD = 202; //解散公会
	Msg.RES_HEARTBEAT = 0; //发送心跳到gate
	Msg.RES_SELECT_GATE = 1; //选择gate_server
	Msg.RES_CONNECT_GATE = 2; //登录gate_server
	Msg.RES_ROLE_INFO = 3; //角色信息更新
	Msg.RES_ERROR_CODE = 4; //返回错误号
	Msg.RES_MAIL_INFO = 5; //邮件信息更新
	Msg.RES_PICKUP_MAIL = 6; //收取附件
	Msg.RES_DEL_MAIL = 7; //删除邮件
	Msg.RES_FETCH_BAG = 8; //获取背包
	Msg.RES_CREATE_GUILD = 201; //创建公会
	Msg.RES_DISSOVE_GUILD = 202; //解散公会
	Msg.SYNC_ERROR_CODE = 0; //同步进程节点错误号
	Msg.SYNC_NODE_INFO = 1; //同步进程节点信息
	Msg.SYNC_GATE_CENTER_VERIFY_TOKEN = 2; //gate通知center校验token
	Msg.SYNC_GATE_GAME_ADD_SESSION = 3; //gate通知game增加session
	Msg.SYNC_GAME_PUBLIC_LOGIN_LOGOUT = 4; //game通知public玩家上线下线
	Msg.SYNC_GAME_GATE_LOGOUT = 5; //game与gate互相通知玩家下线
	Msg.SYNC_GAME_DB_CREATE_PLAYER = 200; //创建玩家
	Msg.SYNC_GAME_DB_LOAD_PLAYER = 201; //加载玩家信息
	Msg.SYNC_GAME_DB_SAVE_PLAYER = 202; //保存玩家信息
	Msg.SYNC_DB_GAME_PLAYER_INFO = 203; //db返回玩家信息
	Msg.SYNC_PUBLIC_DB_CREATE_GUILD = 204; //创建公会
	Msg.SYNC_PUBLIC_DB_LOAD_DATA = 205; //加载public数据
	Msg.SYNC_PUBLIC_DB_SAVE_DATA = 206; //保存public数据
	Msg.SYNC_PUBLIC_DB_DELETE_DATA = 207; //删除public数据
	Msg.SYNC_DB_PUBLIC_DATA = 208; //db返回public数据
	Msg.SYNC_PUBLIC_GAME_GUILD_INFO = 209; //public通知game公会信息
	Msg.SYNC_LOG_PLAYER_LOGOUT = 210; //玩家登出日志
	Msg.HTTP_MODIFY_GOLD = 1; //修改玩家元宝
}
