...
private:
	...
	uint32_t total_length_;		//消息包总长度
	std::string	url_;			//url标识
	std::string	body_content_;	//包体内容
	std::string	referer_;		//包含一个URL，用户从该URL代表的页面出发访问当前请求的页面
	std::string forward_ip_;	//一个用于TRACE方法的请求头标，以指定代理或网关的最大数目，该请求通过网关才得以路由
	std::string user_agent_;	//浏览器类型，如果Servlet返回的内容与浏览器类型有关则该值非常有用
	std::string content_type_;	//内容类型
	uint32_t  content_len_;		//内容长度
	std::string host_;			//初始URL中的主机和端口
};
