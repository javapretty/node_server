...
int file_encrypt(const char* file_path) {
	...
	dest[dest_len] = '\0';
	//数据加密，每次读取16个字节进行加密，加密后赋值到dest数组
	...
	//将加密的数据写入文件，最后一个数组的16字节数据要全部写入文件，解密时候使用，即使有部分数据是无效的
...
int file_decrypt(const char* file_path) {
	...
	//数据解密，每次读取16个字节进行解密，解密后赋值到dest数组
	for(int i = 0; i < encrypt_times; ++i) {
		memset((char*)in, 0, 16);
		memset((char*)out, 0, 16);
		memcpy(in, src + i * 16, 16);
		aes128_ecb_decrypt(in, key, out);
		//解密后的数据为0时候，说明到达文件尾部，舍弃后面的字符
		for(int j = 0; j < 16; ++j) {
			if(out[j] == 0) {
				break;
			}
	...
	//将解密后的数据写入文件，最后一个数组可能有部分为0的无效数据，要去掉
	fwrite(dest, 1, dest_len, file);
	fclose(file);
