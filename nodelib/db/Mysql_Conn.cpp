...
int Mysql_Conn::init() {
	...
	try {
		...
		if (conn_->isClosed()) {
			LOG_ERROR("connect mysql error, url = [%s], user = [%s], password = [%s]", url.c_str(), mysql_pool_->get_username().c_str(), mysql_pool_->get_passwrod().c_str());
			return 1;
		}
...
Mysql_Pool::Mysql_Pool(std::string& pool_name,  std::string& server_ip, uint32_t server_port,
	...
	try {
		...
		LOG_ERROR("mysql驱动连接出错;\n");
	} catch (std::runtime_error&e) {
		LOG_ERROR("mysql运行时错误\n");
	}
}
...
int Mysql_Pool::init() {
	...
	LOG_INFO("db pool: %s, size: %d", pool_name_.c_str(), (int)free_list_.size());
	return 0;
}
	...
	while (free_list_.empty()) {
		if (cur_conn_cnt_ >= max_conn_cnt_) {
			...
			if (ret) {
				...
				free_list_.push_back(mysql_conn);
				cur_conn_cnt_++;
				LOG_INFO("new db connection: %s, conn_cnt: %d", pool_name_.c_str(), cur_conn_cnt_);
			}
		}
	}
...
Mysql_Manager::~Mysql_Manager() {
	for(std::unordered_map<std::string, Mysql_Pool*>::iterator iter = mysql_pool_map_.begin();
...
int Mysql_Manager::init(std::string& server_ip, int server_port, std::string& username, std::string& password, std::string& pool_name, int max_conn_cnt) {
	Mysql_Pool* mysql_pool = new Mysql_Pool(pool_name, server_ip, server_port, username, password, max_conn_cnt);

	if (mysql_pool->init()) {
		delete mysql_pool;
		LOG_FATAL("init mysql db failed, server_ip:%s, server_port:%d, username:%s, password:%s, pool_name:%s",
				server_ip.c_str(), server_port, username.c_str(), password.c_str(), pool_name.c_str());
		return -1;
	}
...
Mysql_Conn* Mysql_Manager::get_mysql_conn(std::string& pool_name) {
	std::unordered_map<std::string, Mysql_Pool*>::iterator iter = mysql_pool_map_.find(pool_name);
...
void Mysql_Manager::rel_mysql_conn(Mysql_Conn* conn) {
	...
	std::unordered_map<std::string, Mysql_Pool*>::iterator iter = mysql_pool_map_.find(conn->get_pool_name());
	if (iter != mysql_pool_map_.end()) {
